/* Source: https://github.com/volser/cdk-drag-scroll/blob/master/projects/cdk-drag-scroll/src/lib/drag-scroll.directive.ts */
...
import {DragAutoScroll} from './drag-auto-scroll';

@Directive({
  selector: '[vsDragScroll]',
  exportAs: 'vsDragScroll'
})
export class DragScrollDirective<T = any> implements OnInit, OnDestroy, OnChanges {
  ...
  constructor(private zone: NgZone,
              ...
              @Optional() private _cdkDrag: CdkDrag) {
  }

  ngOnInit() {
    this.cdkDrag =  this.drag || this._cdkDrag;
    this.dragRef = <any>this.cdkDrag['_dragRef'];
    if (this.dragRef) {
  ...
  started() {
    ...
    this.autoScroll = new DragAutoScroll(this.scrollContainer);
