...
import { AuthenticationService } from '../core/services/authentication.service';
  ...
  @Component({
    ...
    selector: 'ntk-percent-input',
    templateUrl: './ntk-number-input.component.html',
    styleUrls: ['./ntk-number-input.component.scss'],
    providers: [{provide: MatFormFieldControl, useExisting: InputPercentComponent}],
  ...
  // The value used to bind to this componet is the object with foramt {Value: number, IsPercent: boolean } : EX: {Value: 10, IsPercent: true}
  
  export class InputPercentComponent extends _MatInputMixinBase implements MatFormFieldControl<number>, OnChanges
      ...
      //ngControl = null;
      errorState = false;
      controlType = 'ntk-percent-input';
      id = `ntk-percent-input-${InputPercentComponent.nextId++}`;
      describedBy = '';
      ...
      ngOnChanges(changes: SimpleChanges): void {
        let needReDisplay = changes['numberDecimals'] || changes['minDecimals'];
        if (needReDisplay) 
            this.formatValueToView();
      ...
      private formatValueToView() {
        ...
        if (this._value) {
            this.displayText = this.utilityService.transform(this._value.Value, this.authenticationService.groupSep
                , this.authenticationService.decimalSep
                , numberDecimals, true, minDecimals);
            
            if (this._value.IsPercent) 
                this.displayText += "%";
        }
        else {
      ...
      private parseAndFormatValue(stringValue: string) {
        ...
        if (String.IsNullOrWhiteSpace(stringValue)) {
            if (this.allowNull) {
                ...
                this._value = {Value:0, IsPercent: false};
                this.formatValueToView();
            }
        }
        else {

            let isPercent = stringValue.indexOf("%") >= 0;
            let replaceValue = stringValue.replace(/%/g, '');
            let numberValue = String.IsNullOrWhiteSpace(replaceValue) ? 0 : this.utilityService.tryParseFloat(replaceValue);
            ...
            if (this.utilityService.isNumber(numberValue) && !isNaN(numberValue)) {
                ...
                if (this.maxValue !== null && numberValue > this.maxValue) {
                    ...
                    this._value = {Value: numberValue, IsPercent: isPercent};
                    this.formatValueToView();
                }
            }
            else {
      ...
      onInputKeyDown($event: KeyboardEvent) {
        ...
        // 53 = %
        allowkeyCodes = [190, 110, 188, 53]; // this.authenticationService.decimalSep === '.' ? [190, 110] : [188]; // 190,110: "." and  180: ","
        ...
        // console.log('charValue =', charValue);
        let valid = /^[0-9]+$/.test(charValue) || charValue === '%';
